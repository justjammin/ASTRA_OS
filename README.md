...
The selected harness owns one coordinator session across the pipeline. Astra uses a read-only scout before Gate 1, runs MAGI judges independently in parallel, and gives each Gate 5 DAG node a focused worker session. The coordinator retains the plan; its budget display suggests compression at 50% but does not enforce native compaction. Workers stay scoped to their task and write boundary. Astra never enables unrestricted auto-run flags; risky commands pause for an explicit response.

Each worker receives its own auto-compaction policy, independent of aggregate token spend. Set `--worker-compact-at 0.5` (the default ratio) and optionally `--worker-context-window 200000` on `astra start` or `astra run`. The window must be the worker model's known capacity, not a requested expansion. MCP callers set `workerCompaction: { ratio: 0.5, contextWindow: 200000 }` on `astra_start`; gate, reviewer, and node packets carry it to Stella.

Claude receives per-process percentage/window settings; Codex receives a token threshold when a known window is supplied; Pi derives its threshold from the active model and applies session-local SDK settings. These configure the native compactor rather than implement an Astra polling loop. Broker worker records report the configured policy, not proof that compaction occurred. [Claude's controls](https://code.claude.com/docs/en/env-vars) apply to its auto-compact window, accept explicit windows of 100K–1M tokens, and cannot raise its native threshold; [Codex's control](https://developers.openai.com/codex/config-reference) sets a token threshold. Native limits still apply. Codex without a known window, and adapters without verified threshold controls, retain native defaults and emit a warning. Stella applies the policy only when its host exposes per-subagent controls and records unsupported cases explicitly. No global host settings are changed.
...
The package is currently version `0.2.5`. A matching semantic version tag runs two release workflows: one packages the tested artifact and creates the GitHub release, while the other publishes the same package version to npm with provenance through trusted publishing.
