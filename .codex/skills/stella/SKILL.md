---
name: stella
description: Use Astra's MCP gates with Codex-native work and subagents for durable, human-approved software-factory runs.
---

# Stella for Codex

Use the Astra MCP server and Codex's native work/subagent facilities. Keep one run per project;
...
3. Call `astra_gate` to get the rendered prompt/contracts from `lib/prompts/gate*.md`, then dispatch the packet with native Codex work/subagents. At Gate 2, aggregate native reviewer findings into the audit artifact; at Gate 5, dispatch native DAG subagents and persist the execution artifact. Respect every declared write boundary.
