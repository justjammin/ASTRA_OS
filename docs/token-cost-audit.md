...
Measured 2026-09-17 UTC with `js-tiktoken` 1.0.21, encoding `o200k_base`, against this repository after the Protect updates. Counts are exact for this encoding and fixture, not provider billing measurements. Other tokenizers differ.
...
| Gate 2: architecture | 2,178 | 1,213 | 3,392 |
| Gate 3: contracts | 1,786 | 1,210 | 2,997 |
| Gate 4: plan | 2,065 | 1,309 | 3,375 |
| Solo Grunt review | 789 | 1,103 | 1,893 |
| MAGI Melchior review | 802 | 1,089 | 1,892 |
| MAGI Balthasar review | 825 | 1,083 | 1,909 |
| MAGI Casper review | 804 | 1,130 | 1,935 |

Gates 1–4 plus solo review contribute **14,568 static instruction tokens** when each packet is sent once. Before this change: 14,119; added review/design instructions: **449 tokens**. MAGI's three reviewer packets total 5,736, versus 1,893 for solo: an additional **3,843**, before the extra reviewers read the same design and produce findings. Solo remains the default.
...
Excluded: host/system instructions, skill discovery, conversation replay, repository/artifact reads, tool responses, external research, generated artifacts, reasoning, retries, the CLI's Gate 1 scout, and optional specialist overlays. Native worker context forwarding can repeat packets. Therefore 14,568 is an instruction contribution, not total run usage or a forecast.

## Reference and transport costs

- Grunt skill + required judge protocol + coverage index: **6,375 tokens** before a domain catalog. Architecture/resilience adds 1,597; object design adds 3,458. Load only relevant domains.
...
- The two new OOP guides total **3,327 tokens**, loaded only for actual object-design questions. They are not embedded in Astra packets. The entire references directory is an on-disk corpus, not default prompt context.
...
1. **Compact embedded JSON schemas.** `schemaText` currently pretty-prints them. Identical JSON semantics in compact form save **1,968 tokens** over Gates 1–4: product 294, architecture 664, contracts 486, plan 524. This is about 13.5% of the measured solo instruction contribution. Verify readability and artifact validity before shipping.
...
4. **Keep solo judging and conditional reference loading.** This already avoids the extra 3,843 static MAGI tokens and additional reviewer reads/outputs. Replace the historical coverage table with a small runtime routing index in a future change while retaining provenance separately. Do not preload the 10,193-token canonical catalog.
...
No cost optimization or model-default change was applied in this update; the requested Protect port added 449 static packet tokens and 423 Grunt entry tokens. The extended guidance stays separate and conditional.
