...
import { compactionPolicy, applyCliCompaction, applyPiCompaction } from "./compaction.mjs";
 ...
 */
export async function invoke(adapterId, options) {
  const compaction = compactionPolicy(adapterId, options.compaction);
  const result = await invokeAdapter(adapterId, compaction ? {
    ...options,
    prompt: `${options.prompt}\n\nIf the native runtime compacts this session, preserve the task, acceptance criteria, exact write boundary, decisions, changed files, verification evidence, and remaining work. Reload authoritative contracts before continuing; compaction never widens permissions.`,
  } : options);
  if (!compaction) return result;
  const applied = result.compaction ?? compaction;
  return { ...result, compaction: applied,
    warning: [result.warning, applied.warning].filter(Boolean).join(" ") || null };
}

async function invokeAdapter(adapterId, {
  prompt,
  role,
  cwd,
  logFile,
  ...
  const modelWarning = !adapter.supportsModel && requestedModel
    ...
    : !adapter.supportsEffort && effort
      ...
      : warning ?? null;
  if (adapter.sdk) return invokePi({ prompt, role, cwd, logFile, timeoutMs, dryRun, sessionId: durableSessionId, requestedModel, effectiveModel, warning: modelWarning, model, effort, budgetTokens, compactAt, compaction, sessionDir, sessionPath, resume, executionPolicy, nativePolicy });
  ...
  applyCliCompaction(adapterId, spec, compactionPolicy(adapterId, compaction));
    ...
    child.on("close", async (code) => {
      ...
      if (allowModelFallback && code !== 0 && requestedModel && modelUnavailable(stderr)) {
        ...
        const fallback = await invoke(adapterId, {
          ...
          budgetTokens,
          compactAt,
          compaction: compaction ? { ...compaction, contextWindow: null } : undefined,
          sessionDir,
          sessionPath,
...
async function invokePi({ prompt, role, cwd, logFile, timeoutMs, dryRun, sessionId, requestedModel, effectiveModel, warning, model, effort, budgetTokens, compactAt, compaction, sessionDir, sessionPath, resume, executionPolicy, nativePolicy }) {
  ...
  const [{ createAgentSession, SessionManager, SettingsManager, createCodingTools, createReadOnlyTools }] = await Promise.all([import("@earendil-works/pi-coding-agent")]);
  if (logFile) await ensureDir(join(logFile, ".."));
  const log = logFile ? createWriteStream(logFile, { flags: "a" }) : null;
  ...
  try {
    ...
    ({ session } = created);
    const appliedCompaction = compaction === undefined ? null : applyPiCompaction(settingsManager, session.model, compaction);
    ...
    return {
      ...
      ...(appliedCompaction ? { compaction: appliedCompaction } : {}),
