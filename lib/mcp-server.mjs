...
import { workerCompaction } from "./compaction.mjs";
...
export const MCP_SERVER_INFO = Object.freeze({ name: "astra-os", version: "0.2.5" });
    ...
    inputSchema: {
      ...
      properties: {
        ...
        workerCompaction: {
          ...
          properties: {
            ratio: { type: "number", minimum: 0.01, maximum: 0.9, default: 0.5 },
            contextWindow: { type: "integer", minimum: 1000, description: "Known worker model context capacity; required for a custom Codex token threshold." },
          },
        },
...
function runContext(run, input) {
  ...
  return {
    ...
    workerCompaction: workerCompaction(ledger.meta.workerCompaction),
...
function validateStart(input) {
  ...
  getRuntime(runtime);
  let compaction;
  try { compaction = workerCompaction(input.workerCompaction); }
  catch (error) { throw invalidParams(error.message); }
  return { intent, agent, judge, runtime, compaction };
}

async function startOperation(input, baseCwd) {
  const { intent, agent, judge, runtime, compaction } = validateStart(input);
  const cwd = normalizedCwd(input, baseCwd);
...
async function gateOperation(input, baseCwd) {
  ...
  const result = {
    ...
    gate: { id: gate.id, n: gate.n, name: gate.name, status: run.ledger.gates[gate.id]?.status ?? "pending" },
    validation,
    workerCompaction: ctx.workerCompaction,
