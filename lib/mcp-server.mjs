...
import { runDesignGate, runExecution } from "./pipeline.mjs";
...
export const MCP_SERVER_INFO = Object.freeze({ name: "astra-os", version: "0.1.0" });
...
const TOOL_DEFINITIONS = Object.freeze([
  ...
  {
    name: "astra_status",
    description: "Read the current Astra ledger, gate validation, and pending interaction.",
...
async function runOperation(input, baseCwd, dependencies) {
  ...
  const requestedGate = input.gate === undefined ? undefined : gateId(text(input.gate, "gate", { required: true }));
  ...
  for (;;) {
    ...
    if (!result.ok || !all) {
      return { ok: Boolean(result.ok), gate: gate.id, gates, ...summarizeLedger(ledger, run.root), ledger };
    }
...
async function approveOperation(input, baseCwd) {
  ...
  const run = await resolveRun(input, baseCwd);
  const requestedGate = input.gate === undefined ? run.ledger.phase : gateId(text(input.gate, "gate", { required: true }));
  const verdict = await checkGate(run.root, requestedGate);
  if (!verdict.ok) {
  ...
  switch (method) {
    case "initialize":
      return {
        ...
        instructions: "Use astra_start before astra_run. Gate approval requires human:true; interaction responses require resumeToken.",
      };
    case "ping":
      return {};
    case "tools/list":
...
export async function serveStdio({ server = createMcpServer(), input = process.stdin, output = process.stdout, maxMessageBytes = MAX_MESSAGE_BYTES } = {}) {
  ...
  try {
    ...
    // Keep this reference alive for embedders that replace stdout while the transport is running.
