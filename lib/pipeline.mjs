...
import { classifyCommand, createInteraction, emitParentEvent, waitForInteraction } from "./policy.mjs";
...
export async function runCommand(command, cwd, file, dryRun = false, interaction = {}) {
  if (dryRun) return Promise.resolve({ code: 0, stdout: "", stderr: "" });
  const policy = classifyCommand({ agent: interaction.agent, command, cwd, source: "gate5-node" });
  if (policy.decision !== "allow") {
    const request = await createInteraction(interaction.root, {
      runId: interaction.runId,
      kind: "command-approval",
      agent: interaction.agent,
      source: "gate5-node",
      risk: policy.risk,
      summary: `Approval required: ${policy.reasons.join(", ")}`,
      command: policy.normalizedCommand,
      cwd,
      node: interaction.node,
    });
    await emitParentEvent(interaction.root, request);
    const response = await waitForInteraction(interaction.root, { timeoutMs: interaction.timeoutMs });
    if (response.status !== "approved") {
      return { code: 125, stdout: "", stderr: `blocked by command policy: ${response.response?.value ?? "approval denied"}`, blocked: true };
    }
  }
