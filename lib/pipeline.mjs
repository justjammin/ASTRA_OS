...
export async function prepareGatePrompt(ctx, gateId) {
  ...
  return {
    ...
    workerCompaction: workerCompaction(ctx.workerCompaction),
...
export async function prepareReviewerPackets(ctx) {
  ...
  return Promise.all(personas.map(async (persona) => {
    ...
    return {
      compaction: workerCompaction(ctx.workerCompaction),
      name: persona.name,
      lens: persona.lens,
      brief: persona.brief,
...
async function runProductScout(ctx, { log }) {
  ...
  try {
    ...
    await finishWorker(ctx.root, workerId, { ok: turn.code === 0, sessionId: turn.sessionId, usage: turn.usage, warning: turn.warning, compaction: turn.compaction });
