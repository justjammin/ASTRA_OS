...
export async function beginWorker(root, { id = randomUUID(), kind, harness, model, effort, task, compaction }) {
  await mutateSession(root, (session) => {
    session.workers.push({ id, kind, harness, model, effort, task, compaction, status: "running", startedAt: nowIso() });
  });
  return id;
}
...
export async function finishWorker(root, id, result = {}) {
  return mutateSession(root, (session) => {
    ...
    Object.assign(worker, {
      ...
      compaction: result.compaction ?? worker.compaction ?? null,
